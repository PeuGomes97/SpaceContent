...
@app.route("/")
def homepage():
    """Homepage, Redirect to register user"""
...
def register():
    """Register user, create form and handle submission"""
...
def result_for_apod(user_id):
    ...
    # Validate form for filtered results based on user's search
    ...
    # Default result for when users sees page for the first time
...
def mars_photos(user_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if data is None:
            ...
            photos = data['photos']
            photos = photos[:10]
            # images_info = [{'img_src': photo['img_src'], 'rover_name': photo['rover']['name']} for photo in photos]
        ...
        # Logic to create a list based on which pictures available are already added to the favorites list
...
def delete_favorite(user_id, favorite_id):
    """Delete a user's favorite image"""

    if "user_id" not in session or user_id != session['user_id']:
        raise Unauthorized()
    
    # Handle picture to be deleted based on the click of "Delete" button at Favorites route
    ...
    # Retornar uma resposta indicando sucesso na remoção
...
def add_to_favorites(user_id):
    ...
    # Handles favorited images, adding to user's list, based on the click of "favorite" button for each image

    image_url = request.form.get('image_url')  # Pega a URL da imagem do formulário
    ...
    return redirect(url_for('mars_photos', user_id=user_id))
